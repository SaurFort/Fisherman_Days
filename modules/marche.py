    ...
    def vente(self, glaciere: Glaciere) -> int:
        ...
        for poisson in compte:
            if poisson == "Aiglefin":
                ...
                self.prix_aiglefin = round((self.prix_aiglefin * 0.98) * compte[poisson], 2)
            elif poisson == "Thon":
                argent += compte[poisson] * self.prix_thon
                self.prix_thon = round((self.prix_thon * 0.98) * compte[poisson], 2)
            elif poisson == "Merlin":
                argent += compte[poisson] * self.prix_merlin
                self.prix_merlin = round((self.prix_merlin * 0.98) * compte[poisson], 2)
            elif poisson == "Fugu":
                argent += compte[poisson] * self.prix_fugu
                self.prix_fugu = round((self.prix_fugu * 0.98) * compte[poisson], 2)
            else:
                argent += compte[poisson] * self.prix_maquereau
                self.prix_maquereau = round((self.prix_maquereau * 0.98) * compte[poisson], 2)
